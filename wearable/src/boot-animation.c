...
int xready_cb(keynode_t * node, void *user_data)
{
	...
	if (vconf_get_bool(VCONFKEY_SETAPPL_SOUND_STATUS_BOOL, &soundon) < 0)
		fprintf(stderr, "VCONFKEY_SETAPPL_SOUND_STATUS_BOOL ==> FAIL!!");

	if (init_animation(type, args->msg) != EXIT_SUCCESS) {
		fprintf(stderr, "Exit boot-animation\n");
		return EXIT_FAILURE;
	}
...
int check_csc_data(char *filepath, int type)
{
	...
	if (!f) {
		fprintf(stderr, "file doen't exist : %s\n", filepath);
		return -1;
	}

	fprintf(stderr, "found csc file : %s\n", filepath);
	fclose(f);
